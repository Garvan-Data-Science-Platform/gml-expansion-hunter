"""
Submit the WDL pipeline to cromwell via hail batch
"""
...
OUTPUT_PREFIX = f'expansion-hunter/{JOB_NAME}'
DRIVER_IMAGE = _config['workflow']['driver_image']

b = get_batch()

input_prefix = 'ExpansionHunter'

input_dict = {
    f'{input_prefix}.{k}': v
    for k, v in _config['expansion_hunter'].items()
}

submit_j, workflow_id_file = submit_cromwell_workflow(
    b=b,
    job_prefix='expansion-hunter',
    dataset=DATASET,
    access_level=ACCESS_LEVEL,
    workflow='expansion_hunter.wdl',
    output_prefix=OUTPUT_PREFIX,
    cwd='wdl/',
    input_dict=input_dict,
    libs=[
